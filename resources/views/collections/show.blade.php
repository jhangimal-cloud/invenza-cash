...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold text-slate-800">{{ $collectionTracking->title }}</h1>
                <p class="text-sm text-slate-500">{{ $collectionTracking->tracking_code }}</p>
            </div>
            <a href="{{ route('collections.index') }}" class="rounded-lg bg-white border border-slate-300 px-4 py-2 text-slate-700 hover:bg-slate-50">
                Volver a la bandeja
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1 space-y-4">
                <div class="bg-white rounded-xl border border-slate-200 p-5 space-y-3">
                    <h2 class="font-bold text-slate-800">Resumen</h2>
                    ...
                    <div>
                        ...
                        <div class="font-semibold text-slate-800">{{ $collectionTracking->receivable->customer_name }}</div>
                    ...
                    <div>
                        ...
                        <div class="font-semibold text-slate-800">{{ $collectionTracking->responsible?->name ?: 'Sin responsable' }}</div>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <div class="text-xs text-slate-500">Saldo</div>
                            <div class="font-bold text-slate-800">${{ number_format((float) $collectionTracking->receivable->balance, 2) }}</div>
                        </div>
                        <div>
                            <div class="text-xs text-slate-500">Prioridad</div>
                            <div class="font-bold text-slate-800">{{ ucfirst($collectionTracking->priority) }}</div>
                        </div>
                    </div>
                ...
                <form method="POST" action="{{ route('collections.activities.store', $collectionTracking) }}" class="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
                    @csrf
                    <h2 class="font-bold text-slate-800">Registrar gestión</h2>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Tipo</label>
                        <select name="activity_type" id="activityType" class="w-full rounded-lg border-slate-300" required
                    ...
                    <div id="promiseFields" class="hidden space-y-3 rounded-lg bg-violet-50 border border-violet-200 p-3">
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Monto prometido</label>
                            <input type="number" step="0.01" min="0" name="promised_amount" class="w-full rounded-lg border-slate-300">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Fecha prometida de pago</label>
                            <input type="date" name="promised_payment_date" class="w-full rounded-lg border-slate-300">
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Cambiar estado</label>
                        <select name="new_status_id" class="w-full rounded-lg border-slate-300">
                    ...
                    <div>
                        ...
                        <input type="text" name="subject" class="w-full rounded-lg border-slate-300" placeholder="Ej: Cliente dice que paga el viernes">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Detalle</label>
                        <textarea name="body" rows="4" class="w-full rounded-lg border-slate-300" placeholder="Detalle de la gestión realizada..."></textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Próxima gestión</label>
                        <input type="datetime-local" name="next_follow_up_at" class="w-full rounded-lg border-slate-300">
                    </div>

                    <button class="w-full rounded-lg bg-blue-600 text-white px-4 py-2 hover:bg-blue-700">Guardar gestión</button>
                </form>
            </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                    <div class="p-5 border-b border-slate-200">
                        <h2 class="font-bold text-slate-800">Bitácora</h2>
                        <p class="text-sm text-slate-500">Historial de llamadas, notas, correos, reuniones, promesas de pago y cambios de estado.</p>
                    </div>
                    ...
                    <div class="divide-y divide-slate-100">
                        @forelse($activities as $activity)
                            <div class="p-5">
                                ...
                                @if($activity->body)
                                    <div class="mt-3 text-sm text-slate-700 whitespace-pre-line">{{ $activity->body }}</div>
                                @endif

                                @if($activity->promised_amount)
                                    <div class="mt-3 text-xs text-violet-700 font-semibold bg-violet-50 border border-violet-200 rounded-lg px-3 py-2 inline-block">
                        ...
                        @empty
                            <div class="p-8 text-center text-slate-500">No hay gestiones registradas.</div>
                        @endforelse
                    </div>
                </div>
            </div>
