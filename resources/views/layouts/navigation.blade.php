    ...
    @if(Auth::user()?->is_platform_admin)
        <div class="px-3 pb-3">
            <a href="{{ route('platform.companies.index') }}"
               class="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium border border-amber-400/20 bg-amber-400/10 text-amber-200 hover:bg-amber-400/20 transition
                      {{ request()->routeIs('platform.*') ? 'bg-amber-400/25 ring-1 ring-amber-400/40' : '' }}">
                <span class="w-5 h-5 shrink-0 flex items-center justify-center">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="w-5 h-5">
